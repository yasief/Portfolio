...
if (canvasEl && heroWebGLAvailable()) {
    ...
    `;
        ...
        function cssToVec3(varName) {
            ...
            // THREE.Color does not support 8-digit hex codes with an alpha channel (e.g., #RRGGBBAA).
            // This robustly strips the alpha component if it exists.
            if (hex.length === 9) {
                hex = hex.substring(0, 7); // Convert #RRGGBBAA to #RRGGBB
            }
        ...
        // Position text in the gap between hero card and badges. No clipping.
        function lockHeroCanvasZone() {
            if (!isMobile) { canvasEl.style.clipPath = ''; return; }

            const p0    = document.getElementById('p0');
            const hcEl  = document.querySelector('#p0 .hc');
            const badEl = document.querySelector('#p0 .h-badges');
            if (!p0 || !hcEl || !badEl) return;

            // Use PANEL-relative coords (not viewport) so scroll position doesn't
            // shift the calculation. The canvas fills the panel (inset:0), so canvas
            // local coord 0 = panel top.
            const p0Rect  = p0.getBoundingClientRect();
            const hcRect  = hcEl.getBoundingClientRect();
            const badRect = badEl.getBoundingClientRect();
            const panelH  = p0Rect.height;
            if (panelH <= 0) return;

            const gapTop = hcRect.bottom - p0Rect.top;   // px from panel top
            const gapBot = badRect.top   - p0Rect.top;
            const gapMid = (gapTop + gapBot) / 2;

            // --- 1. Clip canvas to gap zone (canvas-local coords) ---
            const clipT = Math.max(0, Math.round(gapTop));
            const clipB = Math.max(0, Math.round(panelH - gapBot));
            canvasEl.style.clipPath = `inset(${clipT}px 0px ${clipB}px 0px)`;

            // --- 2. Position particle mesh Y to gap center in Three.js world space ---
            // The Three.js camera covers the canvas (== panel). NDC Y: +1 = canvas top, -1 = bottom.
            const fov      = camera.fov * (Math.PI / 180);
            const frustumH = 2 * Math.tan(fov / 2) * camera.position.z;
            const ndcY     = 1 - (2 * gapMid / panelH);
            particleMesh.position.y = ndcY * (frustumH / 2);
        }
        ...
        function onWindowResize() {
            ...
            // Measure text bounding box from particle origins
            let minX = Infinity, maxX = -Infinity;
            let minY = Infinity, maxY = -Infinity;
            const origins = dtOrigins.image.data;
            for (let i = 0; i < PARTICLES; i++) {
                const x = origins[i * 4], y = origins[i * 4 + 1];
                if (x < 9000) {
            ...
            // On mobile: fill 92% of viewport width. On desktop: 50%.
            ...
            // Center X on text bounding box midpoint
            particleMesh.position.x = -((minX + maxX) / 2) * scale;
            if (!isMobile) particleMesh.position.y = 1.0;
        ...
        onWindowResize();
        // Re-lock at multiple points to catch font-load and layout shifts
        setTimeout(lockHeroCanvasZone, 100);
        setTimeout(lockHeroCanvasZone, 400);
        setTimeout(lockHeroCanvasZone, 1000);
        // Re-lock whenever the hero card resizes (e.g. font load, orientation change)
        if (isMobile && typeof ResizeObserver !== 'undefined') {
            const hcEl = document.querySelector('#p0 .hc');
            if (hcEl) new ResizeObserver(lockHeroCanvasZone).observe(hcEl);
        }
