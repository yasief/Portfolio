...
function setupMobileView() {
    window.removeEventListener('wheel', desktopWheelHandler, { passive: false });
    window.removeEventListener('keydown', desktopKeydownHandler);
    window.removeEventListener('touchstart', desktopTouchStart, { passive: true });
    window.removeEventListener('touchend', desktopTouchEnd, { passive: true });
...
(function(){
  ...
  const pts=[];
  for(let i=0;i<(isMobile ? 20 : 50);i++) pts.push({x:Math.random()*W,y:Math.random()*H,vx:(Math.random()-.5)*.4,vy:(Math.random()-.5)*.4,r:Math.random()*1+.3});
  function draw(){
...
(function(){
  ...
  let pkts=[];
  setInterval(()=>{const d=devs[Math.floor(Math.random()*devs.length)];pkts.push({to:d,t:0,col:d.col});}, isMobile ? 1200 : 600);
  function draw(){
    if(!W){resize();return;}
    ctx.clearRect(0,0,W,H);
...
(function(){
  ...
  function scramble(el){
    ...
    let frame=0;
    const total=52;
    const q=target.split('').map((to,i)=>({
      to, start:Math.floor(Math.random()*total*.35),
      end:Math.floor(total*.55+Math.random()*total*.35), char:''
    }));
    function tick(){
      let out='',done=0;
      q.forEach(r=>{
        if(frame>=r.end){out+=r.to;done++;}
        else if(frame>=r.start){
          if(!r.char||Math.random()<.28)r.char=CHARS[Math.floor(Math.random()*CHARS.length)];
  ...
  document.querySelectorAll('.panel').forEach(p=>{
    new MutationObserver(()=>{
      if(p.classList.contains('active'))
        setTimeout(()=>p.querySelectorAll('[data-scramble]').forEach(scramble),150);
...
(function(){
  const CMDS=[
    {g:'Navigate',ic:'◉',label:'Hero',desc:'Introduction & overview',tag:'01',fn:()=>goTo(0)},
    {g:'Navigate',ic:'⊕',label:'Reflex Game',desc:'Interactive mini-game + 3D',tag:'02',fn:()=>goTo(1)},
    {g:'Navigate',ic:'◈',label:'About Me',desc:'Background & philosophy',tag:'03',fn:()=>goTo(2)},
    {g:'Navigate',ic:'▣',label:'Experience',desc:'LaundryBox · Muffin House',tag:'04',fn:()=>goTo(3)},
    {g:'Navigate',ic:'◎',label:'IT Dashboard',desc:'Live metrics & system status',tag:'05',fn:()=>goTo(4)},
    {g:'Navigate',ic:'⬡',label:'Technical Skills',desc:'Hex grid skill map',tag:'06',fn:()=>goTo(5)},
    {g:'Navigate',ic:'◆',label:'Key Projects',desc:'4 impact projects',tag:'07',fn:()=>goTo(6)},
    {g:'Navigate',ic:'◐',label:'Achievements',desc:'Measurable outcomes',tag:'08',fn:()=>goTo(7)},
    {g:'Navigate',ic:'✉',label:'Contact',desc:'Get in touch',tag:'09',fn:()=>goTo(8)},
    {g:'Actions',ic:'@',label:'Copy Email',desc:'mohamedyasief@gmail.com',tag:'',fn:()=>copy('mohamedyasief@gmail.com','Email copied!')},
    {g:'Actions',ic:'☏',label:'Copy Phone',desc:'+971 50 359 3856',tag:'',fn:()=>copy('+971503593856','Phone copied!')},
    {g:'Actions',ic:'in',label:'LinkedIn',desc:'linkedin.com/in/yasief',tag:'',fn:()=>window.open('https://linkedin.com/in/yasief','_blank')},
    {g:'Actions',ic:'↓',label:'Download Resume',desc:'PDF · Mohamed Yasief',tag:'',fn:()=>{const a=document.createElement('a');a.href='Mohamed_Yasief_IT_Administrator_Resume.pdf';a.download='';a.click();}},
  ];
  ...
  function close(){backdrop.classList.remove('open');input.value='';}
  function move(d){
